...
class RTPMS_OleDB():
    ...
    def __init__(self, rtpms_engine):
        ...
        self.query = "select * from OPENQUERY([JWRTPMS], \'select [time],[tag],[value] from [piarchive]..[piinterp] " \
    ...
    def get_single_value(self, tag_list, time):
        try:
            ...
            query = "select * from OPENQUERY([JWRTPMS], 'select [tag],[time],[value] from [piarchive]..[piinterp] where tag in ({0}) and time = {1}')".format(
